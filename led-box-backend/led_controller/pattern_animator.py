...
class PatternAnimator(ABC):
	...
	def init_led_color_list(self):
		...
		for i in range(0, PatternAnimator.LED_COUNT):
			...
			current_color = self.get_color_tuple_from_led_pattern_colors(current_color_index)
			...
			if (self.led_pattern.color_gradient_length_factor > 0 and led_count_per_color_in_repition > 1):
				...
				previous_color = self.get_color_tuple_from_led_pattern_colors(previous_color_index)

				next_color_index = current_color_index + 1 if current_color_index < color_count - 1 \
					else 0
				next_color = self.get_color_tuple_from_led_pattern_colors(next_color_index)
	...
	def merge_color_tuples(self, color_1: tuple, color_2: tuple, factor: float):
		return (
	...
	def get_color_tuple_from_led_pattern_colors(self, index: int):	
		return (
			self.led_pattern.colors[index].r, 
			self.led_pattern.colors[index].g, 
			self.led_pattern.colors[index].b
		)
